...
export default function MetaInfo({ className }: MetaInfoProps) {
    ...
    return (
        <section ref={sectionRef} className={className}>
            <Button onClick={() => setShowInfo(!showInfo)} iconLeft={faInfoCircle} iconRight={showInfo ? faAngleUp : faAngleDown} label="Show metainfo" variant="tertiary"/>
            <AnimatePresence>
                ...
                >
                    <h3 className={styles.infoBoxTitle}>String Art Meta Information</h3>
                    <p><b>Filename</b>: {metaData?.fileName}</p>
                    <p><b>Pins</b>: {metaData?.pins}</p>
                    <p><b>Lines</b>: {metaData?.lines}</p>
                </motion.div>}
            </AnimatePresence>
        </section>
    );
